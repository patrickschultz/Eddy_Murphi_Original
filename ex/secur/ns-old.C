/******************************
  Program "ns-old.m" compiled by "Eddy_Murphi Release 3.2.4"
...
#define PROTOCOL_NAME "ns-old"
#define BITS_IN_WORLD 190
#define HASHC
#define PARALLEL
...
char *mu_1_AgentId::values[] = {"InitiatorId_1","ResponderId_1","IntruderId_1",NULL };
...
void mu_1_Message::set_self(char *n, int os)
{
  ...
  mu_key.set_self_2(name, ".key", os + 4 );
  mu_mType.set_self_2(name, ".mType", os + 6 );
  mu_nonce1.set_self_2(name, ".nonce1", os + 8 );
  mu_nonce2.set_self_2(name, ".nonce2", os + 10 );
  mu_address.set_self_2(name, ".address", os + 12 );
}
...
class mu_1_InitiatorStates: public mu__byte
{
 public:
  ...
  {
    if (val.defined())
      return ( s << mu_1_InitiatorStates::values[ int(val) - 7] );
    else return ( s << "Undefined" );
  };

  mu_1_InitiatorStates (char *name, int os): mu__byte(7, 9, 2, name, os) {};
  mu_1_InitiatorStates (void): mu__byte(7, 9, 2) {};
  mu_1_InitiatorStates (int val): mu__byte(7, 9, 2, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
class mu_1_ResponderStates: public mu__byte
{
 public:
  ...
  {
    if (val.defined())
      return ( s << mu_1_ResponderStates::values[ int(val) - 10] );
    else return ( s << "Undefined" );
  };

  mu_1_ResponderStates (char *name, int os): mu__byte(10, 12, 2, name, os) {};
  mu_1_ResponderStates (void): mu__byte(10, 12, 2) {};
  mu_1_ResponderStates (int val): mu__byte(10, 12, 2, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
void mu_1__type_1::set_self( char *n, int os)
{
  ...
  name = n;
  for(i = 0; i < 10; i++)
    array[i].set_self(tsprintf("%s{%d}", n,i), i * 14 + os);
  k = os + i * 14;
  for(i = 0; i < 10; i++)
...
void mu_1_Intruder::set_self(char *n, int os)
{
  ...
  mu_messages.set_self_2(name, ".messages", os + 6 );
}
...
void mu_1__type_3::set_self( char *n, int os)
  {
    int i=0;
    name = n;
array[i].set_self_ar(n,"InitiatorId_1", i * 4 + os);i++;
}
...
void mu_1__type_4::set_self( char *n, int os)
  {
    int i=0;
    name = n;
array[i].set_self_ar(n,"ResponderId_1", i * 4 + os);i++;
}
...
void mu_1__type_5::set_self( char *n, int os)
  {
    int i=0;
    name = n;
array[i].set_self_ar(n,"IntruderId_1", i * 166 + os);i++;
}
...
const int mu_M_NonceAddress = 4;
const int mu_M_NonceNonceAddress = 5;
const int mu_M_Nonce = 6;
const int mu_I_SLEEP = 7;
const int mu_I_WAIT = 8;
const int mu_I_COMMIT = 9;
const int mu_R_SLEEP = 10;
const int mu_R_WAIT = 11;
const int mu_R_COMMIT = 12;
/*** Variable declaration ***/
mu_1__type_2 mu_net("net",0);

/*** Variable declaration ***/
mu_1__type_3 mu_ini("ini",16);

/*** Variable declaration ***/
mu_1__type_4 mu_res("res",20);

/*** Variable declaration ***/
mu_1__type_5 mu_int("int",24);
...
bool mu__boolexpr7;
bool mu__boolexpr8;
  if (!(!((mu_j>=3 && mu_j<=3)))) mu__boolexpr8 = FALSE ;
  else {
...
bool mu__boolexpr11;
bool mu__boolexpr12;
  if (!(!((mu_j>=3 && mu_j<=3)))) mu__boolexpr12 = FALSE ;
  else {
...
bool mu__boolexpr14;
  if (!(!((mu_k>=3 && mu_k<=3)))) mu__boolexpr14 = FALSE ;
  else {
...
/*** end multisetcount 2 declaration ***/
  ...
  void NextRule(unsigned & what_rule)
  {
    ...
    mu_k.unionassign(r % 3);
    r = r / 3;
    while (what_rule < 273 && mu_j.value()<10 )
      {
        if ( ( TRUE  ) ) {
bool mu__boolexpr16;
...
public:
  ...
  return !((mu_net[mu_j].mu_source>=3 && mu_net[mu_j].mu_source<=3));
  }
  ...
  void NextRule(unsigned & what_rule)
  {
    ...
    mu_j.value((r % 1) + 0);
    r = r / 1;
    while (what_rule < 274 && mu_j.value()<1 )
      {
        if ( ( TRUE && mu_net.in(mu_j) ) ) {
              if (!((mu_net[mu_j].mu_source>=3 && mu_net[mu_j].mu_source<=3))) {
                if ( ( TRUE  ) )
                  return;
                else
                  what_rule++;
...
bool mu__boolexpr26;
  ...
  mu__boolexpr25 = ((mu_net[mu_j].mu_source>=3 && mu_net[mu_j].mu_source<=3)) ; 
}
  ...
  void NextRule(unsigned & what_rule)
  {
    ...
    mu_i.value((r % 1) + 2);
    r = r / 1;
    while (what_rule < 275 && mu_j.value()<1 )
      {
        if ( ( TRUE && mu_net.in(mu_j) ) ) {
bool mu__boolexpr27;
bool mu__boolexpr28;
  ...
  mu__boolexpr27 = ((mu_net[mu_j].mu_source>=3 && mu_net[mu_j].mu_source<=3)) ; 
}
              if (mu__boolexpr27) {
                if ( ( TRUE  ) )
                  return;
...
bool mu__boolexpr30;
  ...
  mu__boolexpr29 = ((mu_net[mu_j].mu_source>=3 && mu_net[mu_j].mu_source<=3)) ; 
}
  ...
  void NextRule(unsigned & what_rule)
  {
    ...
    mu_i.value((r % 1) + 2);
    r = r / 1;
    while (what_rule < 276 && mu_j.value()<1 )
      {
        if ( ( TRUE && mu_net.in(mu_j) ) ) {
bool mu__boolexpr31;
bool mu__boolexpr32;
  ...
  mu__boolexpr31 = ((mu_net[mu_j].mu_source>=3 && mu_net[mu_j].mu_source<=3)) ; 
}
              if (mu__boolexpr31) {
                if ( ( TRUE  ) )
                  return;
...
bool mu__boolexpr34;
  ...
  void NextRule(unsigned & what_rule)
  {
    ...
    mu_i.value((r % 1) + 1);
    r = r / 1;
    while (what_rule < 277 && mu_j.value()<1 )
      {
        if ( ( TRUE && mu_net.in(mu_j) ) ) {
bool mu__boolexpr35;
bool mu__boolexpr36;
  ...
  mu__boolexpr35 = ((mu_net[mu_j].mu_source>=3 && mu_net[mu_j].mu_source<=3)) ; 
}
              if (mu__boolexpr35) {
                if ( ( TRUE  ) )
                  return;
...
void SetNextEnabledRule(unsigned & what_rule)
{
  ...
  if (what_rule>=243 && what_rule<273)
  ...
  if (what_rule>=277 && what_rule<280)
    { R6.NextRule(what_rule);
      if (what_rule<280) return; }
}
bool Condition(unsigned r)
{
  category = CONDITION;
  if (r<=242) return R0.Condition(r-0);
  if (r>=243 && r<=272) return R1.Condition(r-243);
  if (r>=273 && r<=273) return R2.Condition(r-273);
  if (r>=274 && r<=274) return R3.Condition(r-274);
  if (r>=275 && r<=275) return R4.Condition(r-275);
  if (r>=276 && r<=276) return R5.Condition(r-276);
  if (r>=277 && r<=279) return R6.Condition(r-277);
Error.Notrace("Internal: NextStateGenerator -- checking condition for nonexisting rule.");
}
void Code(unsigned r)
{
  if (r<=242) { R0.Code(r-0); return; } 
  if (r>=243 && r<=272) { R1.Code(r-243); return; } 
  if (r>=273 && r<=273) { R2.Code(r-273); return; } 
  if (r>=274 && r<=274) { R3.Code(r-274); return; } 
  if (r>=275 && r<=275) { R4.Code(r-275); return; } 
  if (r>=276 && r<=276) { R5.Code(r-276); return; } 
  if (r>=277 && r<=279) { R6.Code(r-277); return; } 
}
int Priority(unsigned short r)
{
  if (r<=242) { return R0.Priority(); } 
  if (r>=243 && r<=272) { return R1.Priority(); } 
  if (r>=273 && r<=273) { return R2.Priority(); } 
  if (r>=274 && r<=274) { return R3.Priority(); } 
  if (r>=275 && r<=275) { return R4.Priority(); } 
  if (r>=276 && r<=276) { return R5.Priority(); } 
  if (r>=277 && r<=279) { return R6.Priority(); } 
}
char * Name(unsigned r)
{
  if (r<=242) return R0.Name(r-0);
  if (r>=243 && r<=272) return R1.Name(r-243);
  if (r>=273 && r<=273) return R2.Name(r-273);
  if (r>=274 && r<=274) return R3.Name(r-274);
  if (r>=275 && r<=275) return R4.Name(r-275);
  if (r>=276 && r<=276) return R5.Name(r-276);
  if (r>=277 && r<=279) return R6.Name(r-277);
  return NULL;
}
};
const unsigned numrules = 280;

/********************
  parameter
 ********************/
#define RULES_IN_WORLD 280
