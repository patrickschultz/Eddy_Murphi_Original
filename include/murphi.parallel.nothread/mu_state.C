...
void dynBitVec::clear_bitvector()
{
	memset( v, 0, NumBytes() );
	return;
}
...
StatePtr::StatePtr(state* s, NodeId n) { sCheck(); pointer=s; node=n; }
StatePtr::StatePtr(unsigned long l, NodeId n) { lCheck(); number=l; node=n; }

void StatePtr::set(state* s, NodeId n) { sCheck(); pointer=s; node=n; }
void StatePtr::set(unsigned long l, NodeId n) { lCheck(); number=l; node=n; }
void StatePtr::clear() {
#ifdef HASHC
  if (args->trace_file.value) 
    number=0;
...
unsigned long StatePtr::getNumber() { lCheck(); return number; }
NodeId StatePtr::getNode() { return node; }

StatePtr StatePtr::previous() {   // return StatePtr to previous state
#ifdef HASHC
  if (args->trace_file.value)
    return StatePtr(TraceFile->read(number, node)->predNumber, 
		    TraceFile->read(number, node)->predNode);
  else
#endif
    // remark: this does not work for the parallel version
    return StatePtr(pointer->previous.pointer,
...
bool StatePtr::isStart() {   // check if I point to a startstate
#ifdef HASHC
  if (args->trace_file.value) {
    if (TraceFile->read(number, node)->predNumber==0) return TRUE;
    return FALSE;
  }
  else 
#endif
  {
    // remark: this does not work for the parallel version
    if (pointer->previous.pointer==NULL) return TRUE;
    return FALSE;
  }
}
...
bool StatePtr::compare(state* s) {   // compare the state I point to with s
  ...
  if (args->trace_file.value) {
    ...
    return ( c1==TraceFile->read(number,node)->c1 &&
             c2==TraceFile->read(number,node)->c2 );
  }
  else
#endif
    return (StateCmp(pointer,s)==0);
}
...
: table_size (table_size), num_elts(0), num_collisions(0)
{
#ifndef HASHC
...
bool 
state_set::simple_was_present( state *& in )   // changes for PMu
...
// the state-insertion is done in two steps: search and insertion
  ...
  // write trace info
  if (args->trace_file.value)
    TraceFile->write(c1, c2, in->previous.getNumber(), 
			     in->previous.getNode());
...
bool 
state_set::simple_was_present( state *& in, bool valid )
/* changes in to point to the first state found with that pattern. */
/* returns true iff the state was present in the hash table;
 * Otherwise, returns false and inserts the state. 
 * If valid is true, then curstate must point to a state with its
 * hashkeys computed, to be used in diff hashing */
/* Algorithms directly from Andreas\' code. He cites CLR 235, 236. */
// Uli: pitfall: shift operators yield undefined values if the right
//               operand is equal to the length in bits of the left
//               operand (see ARM, pg.74)
...
bool 
state_set::was_present( state *& in )   // changes for PMu
{
  ...
  if (args->multiset_reduction.value
      ...
      in->MultisetSort();
  return simple_was_present( in );
}

bool 
state_set::was_present( state *& in, bool valid )
{
  if (args->symmetry_reduction.value)
    in->Normalize();
  if (args->multiset_reduction.value
      && !args->symmetry_reduction.value)
      in->MultisetSort();
  return simple_was_present( in, valid );
}
