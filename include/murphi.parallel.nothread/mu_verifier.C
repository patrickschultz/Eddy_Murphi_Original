...
//   else
  if ( args->main_alg.mode == argmain_alg::Verify_bfs )
    ...
    {
      Algorithm->verify_dfs();	/* never called with PARALLEL */
    }
  else if ( args->main_alg.mode == argmain_alg::Simulate )
    {
      Algorithm->simulate();
    }
  else if ( args->main_alg.mode == argmain_alg::Heuristic1 )
  	{
		Algorithm->heuristic1();
  	}
  else if ( args->main_alg.mode == argmain_alg::Heuristic2 )
  	{
  ...
  else if ( args->main_alg.mode == argmain_alg::Heuristic4 )
  	{
		Algorithm->heuristic4();
  	}
