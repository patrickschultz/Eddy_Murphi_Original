...
/* Hemanth: Now g++ recognizes strstr, so use it rather than this. */
#if 0
const char* 
  ...
  unsigned long exactNumStates = (unsigned long)
    ( (double) bytes * 8 /
      ( state_set::bits_per_state() +
        args->fracqsize.value * 8 * state_queue::BytesForOneState()));
...
{
  setofrules retval;
  for ( unsigned short r=0; r<numrules; r++)
    {
...
/***************************   // added by Uli
  random number generator      // changes for PMu
  ***************************/
...
randomGen::randomGen(bool commonSeed)
{
  if (commonSeed)           // common seed on each node
    value = CommonSeed(randomSeed());
  else                      // separate seed on each node
    value = randomSeed();
}
    
unsigned long randomGen::randomSeed()
{
  struct timeval tp;
  struct timezone tzp;
  unsigned long seed;
  ...
  // remark: if clocks on each node are synchronized, the following
  //         might not be random enough
  gettimeofday(&tp, &tzp);
  seed = ((unsigned long)(tp.tv_sec^tp.tv_usec) * 2654435769) >> 1;

  return (seed==0 ? 46831694 : seed);
}
