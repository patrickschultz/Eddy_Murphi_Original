...
public:
  ...
  void StartCountingCurstate() { num_error_curstate = 0; }
  int ErrorNumCurstate() { return num_error_curstate; }
  bool Phase2Done() { return phase_2_done; }

  void SpecialPhase() { oldphase = phase; phase = 3; };
  void NormalPhase() { phase = oldphase; };

  void ResetErrorFlag() { phase = 3; has_error = FALSE; }
  bool NoError() { return !has_error; };

  int NumError() { return num_errors; };
...
public:
  ...
  virtual char *value() { return argv[ index ]; };
  virtual char *nextvalue() { if (index+1>=argc) return ""; else return argv[ index+1 ]; };
  virtual string_iterator& next() { index++; return *this; }
  virtual bool done() { return (index >= argc); }
  virtual void start() { };
};
...
public:
  ...
  :old(s), current(NULL) { start(); };
  virtual char *value() { return current; };
  virtual string_iterator& next()
  { current = strtok(NULL," "); return *this; }
  virtual bool done() { return (current == NULL); }
  virtual void start()
...
private:
  ...
  char * name;
public:
  argmain_alg(MainAlgorithmtype t, char *n) : mode(t), initialized(FALSE), name(n) {};
  ~argmain_alg() {};
  void set(MainAlgorithmtype t) 
  { if (!initialized) 
      { initialized = TRUE; mode = t; }
    else if (mode != t)
...
private:
  ...
  char * name;
public:
  argsym_alg(SymAlgorithmType t, char *n) : mode(t), initialized(FALSE), name(n) {};
  ~argsym_alg() {};
...
private:
  ...
  char * name;
public:
  argdouble(double val, char * n) : value(val), initialized(FALSE), name(n) {};
  ~argdouble() {};
  void set(double val) 
  { if (!initialized) 
      { initialized = TRUE; value = val; }
...
public:
  ...
  // for heurustic1, heuristic2
  argbool send_rw;
  argnum send_rw_interval;
  argnum total_initial_rw_steps;
  
  // for heuristic2
  argdouble fracqsize;	// ratio of the queue size to the HT size
  argnum init_bfs_depth;
  argnum bounded_mem;   // mem allocated for bounded BFS HT
  argnum bounded_bfs_depth;
  argnum bounded_bfs_interval;
 
  // for heuristic3
  argdouble fracfrontsize; // ratio of frontier array size to HT size
  
  // supporting routines
  argclass(int ac, char** av);
  ~argclass() {};
  void ProcessOptions(string_iterator *options);
  bool ProcessHeuristicOptions(char *option, string_iterator *options,
  								char *temp_str);
  bool Flag(char *arg);
  void PrintInfo( void );
...
public:
  ...
  void print_trace_heuristic1();
  void print_trace_heuristic3();
};
...
// remark: special error handling is required for this class, since
//         an error in accessing the trace info file should NOT cause
//         the verification to stop
...
private:
  ...
  ReadBuffer buf;         // buffer for read
  unsigned long inBufNumber;  // number of state in buffer (0: empty)
  NodeId inBufNode;           // node of state in buffer
  unsigned long last;     // number of last state written
  bool writeStop;         // set after problems to write file

  void writeLong(unsigned long l, int bytes);
  unsigned long readLong(int bytes, bool& error);
...
public:
  ...
  void write(unsigned long c1, unsigned long c2, 
	     unsigned long predNumber, NodeId predNode);
  void writeState(state *s);
  int readNextState(state *s);
  ReadBuffer* readLocal(unsigned long number);
  ReadBuffer* read(unsigned long number, NodeId node);
  void print(int node);
  void rewind_file();
};
#endif
