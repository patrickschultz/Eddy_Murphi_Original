...
friend setofrules conflict(unsigned short rule);
  ...
  unsigned short getnthrule(unsigned short rule)
  {
    unsigned short r=0;
    int i=0;
    while (1)
      if (Get1(i) && r==rule)
	{ Set1(i,FALSE); NumRules--; return (unsigned short) i; }
      else if (Get1(i))
	{ i++; r++; }
      else
	{ i++; } 
