...
public:
  ...
  bool Add(state * s);
  bool Add(state *s, bool valid);
  bool QueueIsEmpty();
  state * QueueTop();
  state * QueueDequeue();
  unsigned short NextRuleToTry();
  void NextRuleToTry(unsigned short r);
  ...
  void AddNumElts(unsigned long a);   // PMu
  unsigned long QueueNumElts();

  long GetCurrentLevel() { return currentLevel; }
  void Reset();
...
public:
  ...
  void AllStartStates();
  void AllStartStates(StateManager *sm);
...
class RuleManager
{
  unsigned short what_rule;       // for execution and info at Error
  ...
  bool AllNextStates(StateManager *sm, setofrules * fire);
  state * NextState();
  randomGen random;   // Uli: random number generator
public:
  RuleManager();
  ...
  state * SeqNextState();
  bool AllNextStates();
  bool AllNextStates(StateManager *sm);
  void ResetRuleNum();
  void SetRuleNum(unsigned short r);
  char * LastRuleName();
  unsigned long NumRulesFired();
  void AddNumRulesFired(unsigned long a);   // PMu
...
int TotalNetworkMessagesExchanged = 0; 
			// total number of actual network messages exchanged

// for Heuristic 1
state_set *rw_ht = NULL;

// for Heuristic 2
state **rw_fronts = NULL;
int size_rw_fronts = 0;
int num_random_walks = 0;
bool start_initial_rw = FALSE;
StateManager *BoundedStateSet = NULL;

// for Heuristic 3
int num_final_random_walks = 0;
int final_rw_interval = 0;
int total_final_rw_steps = 0;
bool start_final_rw = FALSE;
double user_input_time = 0.0;

// for Heuristic 4
state **frontier_array = NULL;
unsigned long max_elts_frontier_array;
unsigned long num_elts_frontier_array = 0UL;
