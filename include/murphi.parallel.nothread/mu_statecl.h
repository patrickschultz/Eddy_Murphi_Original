...
/****************************************    // added by Uli
  class for pointer to previous state        // changes for PMu
  ****************************************/
...
private:
  union {
    state*        pointer;   // real pointer
    unsigned long number;    // state number used in trace info file
  };
  NodeId node;               // node
...
public:
  StatePtr() {}
  StatePtr(state* s, NodeId n);
  StatePtr(unsigned long l, NodeId n);

  void set(state* s, NodeId n);
  void set(unsigned long l, NodeId n);
  void clear();
...
{
public:
  unsigned char bits[ (((4192 + 
  (4*BITS( unsigned char )) - 1 ) / 
  (4*BITS(unsigned char )))*4) ];
  StatePtr previous;   // state from which this state was reached.
#ifdef HASHC
#if defined(ALIGN) || defined(PARALLEL)
  // Uli: only in the aligned version are the hashkeys stored with the state
...
friend void StatePrint(state* s, char* p);   // PMu
...
#endif
friend void SendState(state* s, bool valid);   // PMu
