...
public:
  ...
  virtual void print_capacity( void )
  {
    cout << "\t* Capacity in queue for breadth-first search: "
	 ...
	 << "\t   * Change the command line argument fracqsize to increase this, \
	 				if necessary.\n"; 
	cout.flush();
  }
};
...
{
  unsigned short * nextrule_to_try;
...
public:
  ...
  {
    unsigned long i;
    nextrule_to_try = new unsigned short [ mas ];
    for ( i = 0; i < mas; i++)
      nextrule_to_try[i] = 0;
  };
...
public:
  ...
  // checking the presence of state "in"   // changes for PMu
  bool simple_was_present( state *&in );  
    /* old was_present without checking -sym */
  bool simple_was_present( state *&in, bool valid );  
    /* old was_present without checking -sym */
  bool was_present( state *&in );
    /* checking -sym before calling simple_was_present() */
  bool was_present( state *&in, bool valid );
    /* checking -sym before calling simple_was_present() */
  ...
  // add to the number of elements in the state set, PMu
  void AddNumElts(unsigned long a) { num_elts+=a; };
