  ...
  pno(1.0)
{
  ...
  if (createqueue) 
    { 
      /* To make up for usage of false BytesForOneState value. et */
      qsize = (unsigned long) (gPercentActiveStates * 
      (((double) sizeof(state*))/((double)sizeof(state))) * NumStates);

      if (qsize >= 10)
           queue = new state_queue(qsize);
      else queue = new state_queue(10);
...
bool StateManager::Add(state * s, bool valid, bool permanent)
{
  if ( !the_states->was_present(s, valid, permanent) )
    {
      queue->enqueue(s); //gdp: moved to define s
...
/* gdp: the following two tests added for cached murphi */  
  ...
  if ((args->max_level.value > 0) && (currentLevel > args->max_level.value))
  // stop exploration
  {
      Error.Notrace("Reached depth %u exceedes max depth %u.\n\tStopping exploration to prevent looping",
                    currentLevel,args->max_level.value);
      return (1);
  }


  if ((args->max_collrate.value> 0 ) && (((double) NumOverwrites())/((double) NumElts()) > ((double)args->max_collrate.value/100)))
  // stop exploration
  {
      Error.Notrace("Collision rate %g exceeds max collision rate %g.\n\tStopping exploration to prevent looping.",
                    ((double) NumOverwrites())/((double) NumElts()),((double)args->max_collrate.value/100));
      return(2);
  }
...
unsigned long StateManager::NumOverwrites()
{ 
  return the_states->NumOverwrites();
} 
...
/********************
  $Log: mu_system.C,v $
  Revision 1.3  2001/02/28 20:32:12  enrico
  modified simple_was_present

  Revision 1.2  2001/02/05 08:34:43  enrico
  adj

  Revision 1.1  2001/02/02 09:30:57  enrico
  Initial revision
...
/*
$Log: mu_system.C,v $
Revision 1.4  2001/06/11 16:39:11  giuseppe 
