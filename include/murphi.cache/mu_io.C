...
#define MAX_COLLRATE_PREFIX "-maxc"
...
: argc(ac), argv(av),
  ...
  max_collrate	  (90,"max collision rate"), //gdp: initialize cached murphi options
...
//gdp: cached murphi options begin

      if ( strncmp(option, MAX_COLLRATE_PREFIX, strlen(MAX_COLLRATE_PREFIX) ) == 0 )
        {
          if ( strlen(option) <= strlen(MAX_COLLRATE_PREFIX) ) /* We cannot have a space before the number */
	    {
		...
		Error.Notrace("Unrecognized collision rate limit.  Do '%s -h' for list of valid arguments.",
			      argv[0]);
	    }
          else
	    {
              sscanf( options->value() + strlen(MAX_COLLRATE_PREFIX), "%s", temp_str );
	      if (isdigit(temp_str[0]))
	        sscanf( temp_str, "%u", (unsigned long) &temp );
	      else	  
		Error.Notrace("Unrecognized collision rate limit.  Do '%s -h' for list of valid arguments.",
          ...
          if ((temp < 0) || (temp > 99))  Error.Notrace("Invalid collision rate limit.  Valid values are in range 0-99");
	  max_collrate.set(temp);
          continue;
        };
      ...
      if ( strncmp(option, MAX_LEVEL_PREFIX, strlen(MAX_LEVEL_PREFIX) ) == 0 )
		...
		Error.Notrace("Unrecognized bfs level limit.  Do '%s -h' for list of valid arguments.",
			      argv[0]);
	    }
          else
	    {
              sscanf( options->value() + strlen(MAX_LEVEL_PREFIX), "%s", temp_str );
	      if (isdigit(temp_str[0]))
	        sscanf( temp_str, "%u", (unsigned long) &temp );
	      else	  
		Error.Notrace("Unrecognized bfs level limit.  Do '%s -h' for list of valid arguments.",
          ...
          if (temp < 0)  Error.Notrace("Invalid bfs level limit.  Valid values must be positive or zero");
	  max_level.set(temp);
          continue;
        };

      
//gdp: cached murphi options end
      ...
      if( strcmp( option, SIMULATE_FLAG ) == 0 )
        {
          //gdp: cached murphi is based on the bfs visit!
	  Error.Notrace("Cache can only be used with BFS in this release -- sorry!"); 	  
...
//	   << "\thash compression and breadth first search.\n";
  ...
  {
        cout << "\tThe maximum bfs level depth is " << args->max_level.value << ".\n"
    ...
    { 
      cout << "\t" 
	   ...
	   << SecondsSinceStart() << "s, with "
           ...
           << ", current level " << StateSet->CurrentLevel() -1 << ".\n";
      cout.flush();
    }
}
...
void ReportManager::print_summary(bool prob)
{
  ...
  cout << "\nState Space Explored:\n\n"
       ...
       << SecondsSinceStart() << "s.\n"
       << "\tCollision Rate: " << ((double)StateSet->NumOverwrites() / (double)StateSet->NumElts()) << ".\n"
       << "\tLevels Explored: "<< StateSet->CurrentLevel()-1 << ".\n\n";
