/** -*- C++ -*-
 * @file mu_system.C
 * procedure for analyzing the system
 ...
 *
 * @param[in]	createqueue		Specifies whether or not to create a state queue.
 * @param[in]	NumStates		The number of states in the state table.
 */
StateManager::StateManager(bool createqueue, unsigned long NumStates) :
	NumStates(NumStates), statesCurrentLevel(0), statesNextLevel(0),
			currentLevel(0), pno(1.0) {
#if __WORDSIZE == 64
	...
	if (createqueue) {
		...
		unsigned long qsize = (unsigned long) (gPercentActiveStates
				* (((double) sizeof(state *)) / ((double) (sizeof(state)
						+ sizeof(char)))) * NumStates);
		...
		queue = new state_stack((unsigned long) (gPercentActiveStates
				* NumStates));
	}
#if __WORDSIZE == 64
}
...
/**
 * Destructor. Deletes the queue and the state set.
 */
 ...
 *
 * param[in] qline		The queue of states to add.
 * param[in] lenght		The number of states in the qline.
 *
 * @return				Returns if the states were added or not.
 * @retval		1		States were added.
 * @retval		0		No states added.
 */
int StateManager::AddMultiple(char* qline, int length) {
	int retVal = 0;
	int i = 0;
	static state *s = new state, *s_tmp = s;

	// Make sure our length is a multiple of the state size.
	assert(length%sizeof(class state) == 0);

	// Copy the states to temporary and enqueue.
...
/* IM<b> */
//TODO this will add the state as well if its not present.
bool StateManager::SimpleWasPresent(state * s) {
	/* check if valid is ok */
	bool retVal;

	// TODO Patrick: Why do we copy the state?
...
bool StateManager::Add(state * s, bool valid, bool permanent, bool mayEnqueue) {
...
/* IM<b> */
/**
 * Checks whether we've reached termination.
 * Used in BFS search.
 *
 * @retval		True	We have reached a termination condition.
 * @retval		False	Termination has not been reached.
 */
bool StateManager::ParTerminate() {
	bool ret = FALSE;
	...
	if (Terminate) {
		...
		if (QueueIsEmpty()) {
			...
			if (Terminate)
				...
				if (QueueIsEmpty())
					Error.Notrace("Internal: queue cannot be empty here");
			}
			mutexes->UnLockMutex(MUTEX_TERMINATE);
		}
	}
...
/* IM<b> */
/**
 * Returns the next state in the queue.
 *
 * @param[in]	checked
 * @param[in]	numCurState
 *
...
void StateManager::print_capacity() {
	if (args->main_alg.mode == argmain_alg::Verify_dfs || args->main_alg.mode
			== argmain_alg::Verify_bfs) {
		cout << "\nMemory usage:\n\n";
...
RuleManager::RuleManager() :
	rules_fired(0)
	/* IM<b> */
	, rules_fired_others(0), rules_fired_others_d(0.0),
			is_rules_fired_others_double(FALSE), rules_fired_d(0.0),
			is_rules_fired_double(FALSE)
/* IM<e> */
{
...
/* IM<e> */
{
	...
	static state * originalstate = new state; // buffer for workingstate // useless, always equal to curstate
	state * nextstate;
	...
	for (what_rule = 0; what_rule < numrules; what_rule++) {
		...
		// if (fire->in(what_rule) )
		{
			...
			if (StateCmp(curstate, nextstate) != 0) {
				...
				if (!tracing
						&& !StateSet->Add(nextstate, TRUE, permanent, TRUE)) {
 ...
 ****************************************/
void AlgorithmManager::verify_bfs() {
...
/**
 * @deprecated. Will throw assertion error.
 */
