...
Mutexes::Mutexes(int num_mutexes) :
	NumMutexes(num_mutexes) {
...
int Mutexes::WaitTimedMutex(int mutex, const timespec *wait) {
	if (mutex >= NumMutexes)
		return -1;
	return pthread_cond_timedwait(&signals[mutex], &mutexes[mutex], wait);
	return 0;
}
