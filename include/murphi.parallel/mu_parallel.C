...
//
commManager::commManager(void (*Hashstats)(int, unsigned long *, double *, bool *), int (*Queue)
			 (char*, int), int (*workWaiting)(),
...
: StateLen(stateLen)
{
  ...
  if (numProcs < 2) Error.Notrace("Give at least 2 processors");
  ...
  for (int i = 0; i < numProcs; i++) m_stateReplyCount[i] = 0;
...
unsigned long commManager::memory_allocated()
{
  ...
  return (unsigned long)sizeof(commManager) + 
	 ...
	 (unsigned long)(commQueue::memory_allocated(BuffSize, StateLen, AuxLen, NumBuffs)*(numProcs - 1));
}
...
unsigned long commManager::memory_allocated(int BufSize, int BufCount)
{
  ...
  return (unsigned long)sizeof(commManager) + 
	 ...
	 (args->print_trace.value? (unsigned long)(sizeof(struct final_hashmatrix)*(numProcsStats)) : 0) +
#endif
	 (unsigned long)(commQueue::memory_allocated(BufSize, StateLen, AuxLen, BufCount)*(numProcs - 1));
}
...
commManager::~commManager()
{
  ...
  if (final_stats_buf1 != NULL) delete [] final_stats_buf1;
  if (final_stats_buf1_d != NULL) delete [] final_stats_buf1_d;
  if (final_stats_buf2 != NULL) delete [] final_stats_buf2;
  if (final_stats_buf3 != NULL) delete [] final_stats_buf3;
#ifdef NO_AVAILABLE_BUFFERS_STATS
  no_available_buffers_stats[0] = no_available_buffers_stats[1] = no_available_buffers_stats[2] = 0;
...
void commManager::DisplayMPIError(std::string location, int MPIcode)
{
  ...
  mutexes->LockMutex(MUTEX_PRINT);
  std::cout << myRank << ", " << MPI_Wtime() - iTime 
	    << ", C, MPI Error at: "  << location << " with: " 
	    << std::string(errString) << std::endl;
  std::cout.flush();
  mutexes->UnLockMutex(MUTEX_PRINT);
}
...
void commManager::BroadcastTerminate()
{
  ...
  for (int rank = 0; rank < numProcs; rank++){
    ...
    err = MPI_Isend(result_buf, MESSAGE_TERMINATE_ULONG_LENGTH, MPI_UNSIGNED_LONG, rank, MESSAGE_TERMINATE,
		    MPI_COMM_WORLD, &req);
    if(err) DisplayMPIError("BroadcastTerminate(), MPI_Isend()", err);
  }
}
...
bool commManager::DoSends()
{
  ...
  for(int node = 0; node < numProcs; node++){
    ...
    retVal = queues[node]->GetBufferToCopy(&buf, &len, &index, workerWaiting);
	...
	BroadcastAndEnd();
      ...
      ++Sent;
      if (len > Max_states_in_msg) Max_states_in_msg = len;
      if (len < Min_states_in_msg) Min_states_in_msg = len;
...
bool commManager::RecoverBuffers(bool &hasPendingSends)
{
  ...
  while (index != -1){
	...
	err = queues[reqArray[index].node]->
	  ReturnBuffer(reqArray[index].buffer);
	...
	if (queues[reqArray[index].node]->IsSendPending(reqArray[index].buffer))
	  {
...
void commManager::ReceiveStatesReply()
{
  ...
  err = MPI_Recv(&sendingRank, MESSAGE_STATES_REPLY_INT_LENGTH, MPI_INT, MPI_ANY_SOURCE,
		 MESSAGE_STATES_REPLY, MPI_COMM_WORLD, &stat);
  if (err)
    {
  ...
  // sanity check
  assert(sendingRank == stat.MPI_SOURCE && sendingRank >= 0 && sendingRank < numProcs);
...
void commManager::ReceiveStates()
{
  ...
  MPI_Request req;
  err = MPI_Isend(&myRank, MESSAGE_STATES_REPLY_INT_LENGTH, MPI_INT, stat.MPI_SOURCE, 
		  MESSAGE_STATES_REPLY, MPI_COMM_WORLD, &req);
...
void commManager::End()
{
  ...
  if (!IsRoot()) {
    ...
    if (StateSet->IsNumEltsDouble()) {
      ...
      err1 = MPI_Isend(&final_stats_buf1_d[0], MESSAGE_NUM_STATES_DOUBLE_LENGTH, MPI_DOUBLE, 0,
      ...
      err1 = MPI_Isend(&final_stats_buf1[0], MESSAGE_NUM_STATES_ULONG_LENGTH, MPI_UNSIGNED_LONG, 0,
    ...
    err1 = MPI_Isend(&final_stats_buf2[0], MESSAGE_MESSAGES1_UINT_LENGTH, MPI_UNSIGNED, 0,
		    MESSAGE_MESSAGES1, MPI_COMM_WORLD, &req);
    if(err1){
      DisplayMPIError("MPI_ISend() in End()", err1);
    }
    ...
    err1 = MPI_Isend(&final_stats_buf3[0], MESSAGE_MESSAGES2_DOUBLE_LENGTH, MPI_DOUBLE, 0,
		    MESSAGE_MESSAGES2, MPI_COMM_WORLD, &req);
    if(err1){
      DisplayMPIError("MPI_ISend() in End()", err1);
    }
...
#define TO_BE_READ 3
	...
	err = MPI_Iprobe(rank, MESSAGE_NUM_STATES, MPI_COMM_WORLD, &flag, &status);
	if(err) DisplayMPIError("MPI_Iprobe() in End()", err);
	if(flag) {
	  err1 = MPI_Recv(&final_stats_buf1[rank - 1], MESSAGE_NUM_STATES_ULONG_LENGTH, MPI_UNSIGNED_LONG, rank, MESSAGE_NUM_STATES,
			  MPI_COMM_WORLD, &status);
	  if(err1) DisplayMPIError("End(), MPI_Recv()", err1);
	  StateSet->IncrNumEltsOthers(final_stats_buf1[rank - 1].num_elts);
	  Rules->IncrNumRulesFiredOthers(final_stats_buf1[rank - 1].rules_fired);
	  read++;
	  is_final_stats_buf1_double = FALSE;
	}
	err = MPI_Iprobe(rank, MESSAGE_NUM_STATES_D, MPI_COMM_WORLD, &flag, &status);
	if(err) DisplayMPIError("MPI_Iprobe() in End()", err);
	if(flag) {
	  err1 = MPI_Recv(&final_stats_buf1_d[rank - 1], MESSAGE_NUM_STATES_DOUBLE_LENGTH, MPI_DOUBLE, rank, MESSAGE_NUM_STATES_D,
			  MPI_COMM_WORLD, &status);
	  if(err1) DisplayMPIError("End(), MPI_Recv()", err1);
	  StateSet->IncrNumEltsOthers(final_stats_buf1_d[rank - 1].num_elts);
	  Rules->IncrNumRulesFiredOthers(final_stats_buf1_d[rank - 1].rules_fired);
	  read++;
	  is_final_stats_buf1_double = TRUE;
	}
	err = MPI_Iprobe(rank, MESSAGE_MESSAGES1, MPI_COMM_WORLD, &flag, &status);
	if(err) DisplayMPIError("MPI_Iprobe() in End()", err);
	if(flag) {
	  err1 = MPI_Recv(&final_stats_buf2[rank - 1], MESSAGE_MESSAGES1_UINT_LENGTH, MPI_UNSIGNED, rank, MESSAGE_MESSAGES1, 
			  MPI_COMM_WORLD, &status);
	  if(err1) DisplayMPIError("End(), MPI_Recv()", err1);
	  read++;
	}
	err = MPI_Iprobe(rank, MESSAGE_MESSAGES2, MPI_COMM_WORLD, &flag, &status);
	if(err) DisplayMPIError("MPI_Iprobe() in End()", err);
	if(flag) {
	  err1 = MPI_Recv(&final_stats_buf3[rank - 1], MESSAGE_MESSAGES2_DOUBLE_LENGTH, MPI_DOUBLE, rank, MESSAGE_MESSAGES2, 
			  MPI_COMM_WORLD, &status);
	  if(err1) DisplayMPIError("End(), MPI_Recv()", err1);
	  read++;
	}
...
void commManager::ReceiveTerminate()
{
  ...
  info is needed */
  err = MPI_Recv(result_buf, MESSAGE_TERMINATE_ULONG_LENGTH, MPI_UNSIGNED_LONG, MPI_ANY_SOURCE, MESSAGE_TERMINATE, 
		 MPI_COMM_WORLD, &status);
  if (err){
...
void commManager::RelayToken()
{
  ...
  err = MPI_Isend(&scToken, MESSAGE_TERMINATE_PROBE_INT_LENGTH, MPI_INT, (myRank+1)%numProcs,
		  MESSAGE_TERMINATE_PROBE, MPI_COMM_WORLD, &req);
...
void commManager::ReceiveTermProbe()
{
  ...
  // MPI receive token
  err = MPI_Recv(&scToken, MESSAGE_TERMINATE_PROBE_INT_LENGTH, MPI_INT, MPI_ANY_SOURCE,
		 MESSAGE_TERMINATE_PROBE, MPI_COMM_WORLD,
		 &status);
  if(err){
...
bool commManager::ProcessMessages()
{
  ...
  // MESSAGE_STATES tag
  err = MPI_Iprobe(MPI_ANY_SOURCE, MESSAGE_STATES, 
  ...
  err = MPI_Iprobe(MPI_ANY_SOURCE, MESSAGE_STATES_REPLY,
		   MPI_COMM_WORLD, &flag, &status);
  if (err)
    {
      DisplayMPIError("MPI_Iprove() in ProcessMessages()", err);
      BroadcastAndEnd();
    }
  if (flag)
    {
  ...
  err = MPI_Iprobe(MPI_ANY_SOURCE, MESSAGE_TERMINATE, 
  		      MPI_COMM_WORLD, &flag, &status);
  if (err){
    DisplayMPIError("MPI_Iprobe() in ProcessMessages()", err);
    BroadcastAndEnd();
  ...
  err = MPI_Iprobe(MPI_ANY_SOURCE, MESSAGE_TERMINATE_PROBE, 
		      MPI_COMM_WORLD, &flag, &status);
  if(err){
    DisplayMPIError("MPI_Iprobe() in ProcessMessages()", err);
    BroadcastAndEnd();
...
void commManager::print_capacity()
{
  ...
  cout << "\nParallel Version:\n\tThe communication mechanism is storing " << NumBuffs << " queue lines for each of the " 
    << numProcs << " nodes.\t\n\tEach queue line is able to store " << BuffSize << " states.\n"
    << "The total amount of memory used by the communication handling is " << mem_alloc << " bytes, i.e. "
    << ((double)mem_alloc)/(1<<20) << "MB\n";
  mutexes->UnLockMutex(MUTEX_PRINT);
}
