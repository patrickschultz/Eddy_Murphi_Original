...
/**
 * Returns a pointer to the start of a substring
 * found in the provided string.
 *
 * @return NULL if the match is not found. Otherwise, return
 * 		   a pointer.
 */
const char *
...
/**
 * Sends an memory allocation error if the provided pointer is NULL.
 * Passes the message to the error handler.
 */
...
/**
 * Sends a memory allocation error to the error handler.
 */
 ...
 *
 * @param [in]	n	The positive number to check.
 *
 * @return 			Whether of not the number is prime.
 * @retval true		The number is prime.
 * @retval false	The number is not prime.
 */
...
/* IM<b> */
void CheckMemAvailable(int BufSize, int BufCount, unsigned long bytes)
/* From Andreas\' code.*/
{
	double d_bytes = ((double) bytes - (double) Communicate->memory_allocated(
			BufSize, BufCount));
	if (Communicate->IsRoot() && d_bytes < 1000)
		Error.Notrace(
				"%.2lfMB are not enough, since %.2lfMB are required for the communication handling",
				((double) bytes) / (1 << 20),
				((double) Communicate->memory_allocated(BufSize, BufCount))
						/ (1 << 20));
}
/* IM<e> */
...
{
	/* IM<b> */
	bytes = (unsigned long) ((double) bytes
			- (double) Communicate->memory_allocated());
	/* IM<e> */
	unsigned long exactNumStates = (unsigned long) ((double) bytes * 8
			/ (state_set::bits_per_state() + gPercentActiveStates * 8
					* state_queue::BytesForOneState()));
...
void mu__int::print_diff(state * prevstate) {
...
// Uli: simplified print_diff
void mu__long::print_diff(state * prevstate) {
	/* We assume that prevstate is not null. */
#ifndef ALIGN
	if (prevstate->get(&where) != workingstate->get(&where))
#else
