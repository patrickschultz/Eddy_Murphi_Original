...
public:
  ...
  {				// buffer for read
    ...
    uint32_t c1;                //implementation is dependant on 32 bit ints
    uint32_t c2;
    /* IM<b> */
    int rank;
    /* IM<e> */
...
/* IM<b> */
  ...
  int inBufRank;
  int numBytesRank;
  unsigned long readLong (int bytes, void *p, bool reverse = false);
/* IM<e> */
  ...
  void writeLong (unsigned long l, int bytes, bool reverse = false);
  /* IM<e> */
...
public:
  ...
  void write (uint32_t c1, uint32_t c2, unsigned long previous, int rank);
  const Buffer *read (unsigned long number, int rank);
  /* IM<e> */
};
/* IM<b> */
#endif /* HASHC_TRACE */
