...
hash_function::hash_function (int vsize)
{
  ...
  hashmatrix = (uint32_t *)
             malloc(vec_size*sizeof(uint32_t)*24);
...
/* IM<b> */
hash_function::hash_function (uint32_t *hashmatrix, int vsize)
{
  vec_size = vsize;

  this->hashmatrix = (uint32_t *)
    malloc (vec_size * sizeof (uint32_t) * 24);
  memcpy(this->hashmatrix, hashmatrix, vec_size * sizeof (uint32_t) * 24);

  oldvec = new unsigned char[vec_size];
  for (int i = 0; i < vec_size; i++) {
    oldvec[i] = 0;
  }
  /* Should be changed (here and in other places) */
  key[0] = 0UL;
  key[1] = 0UL;
  key[2] = 0UL;
}
...
void
hash_function::gethashmatrix (uint32_t *hashmatrix)
{
  memcpy(hashmatrix, this->hashmatrix, vec_size * sizeof (uint32_t) * 24);
}
/* IM<e> */
